...
def correct_raster(scan, raster_phase, temporal_fill_fraction, in_place=True):
    ...
    if not isinstance(scan, np.ndarray):
        raise Exception('Scan needs to be a numpy array.')
    if scan.ndim < 2:
        raise Exception('Scan with less than 2 dimensions.')
...
def correct_motion(scan, x_shifts, y_shifts, in_place=True):
    ...
    if np.ndim(y_shifts) != 1 or np.ndim(x_shifts) != 1:
        raise Exception('Dimension of one or both motion arrays differs from 1.')
    if len(x_shifts) != len(y_shifts):
        raise Exception('Length of motion arrays differ.')
    ...
    if reshaped_scan.shape[-1] != len(x_shifts):
        raise Exception('Scan and motion arrays have different dimensions')
